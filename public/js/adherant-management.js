...
/*!*********************************************!*\
  !*** ./resources/js/adherant-management.js ***!
  \*********************************************/
/**
 * Page User List
 */
...
$(function () {
    ...
    var dt_user = dt_user_table.DataTable({
      ...
      columnDefs: [{
        ...
        render: function render(data, type, full, meta) {
          ...
          var $row_output = '<div class="d-flex justify-content-start align-items-center user-name">' + '<div class="d-flex flex-column">' + '<a href="" class="text-body text-truncate"><span class="fw-medium">' + $name + '</span></a>' + '</div>' + '</div>';
          return $row_output;
        }
      }, {
        // User email
        ...
        render: function render(data, type, full, meta) {
          var $entreprise = full['entreprise'];
          return "".concat($entreprise);
        }
      }, {
        // population
        targets: 6,
        ...
        orderable: false,
        render: function render(data, type, full, meta) {
          return '<div class="d-inline-block text-nowrap">' + "<button class=\"btn btn-sm btn-icon edit-record\" data-id=\"".concat(full['id'], "\" data-bs-toggle=\"modal\" data-bs-target=\"#editAdherant\"><i class=\"ti ti-edit\"></i></button>") + "<button class=\"btn btn-sm btn-icon delete-record\" data-id=\"".concat(full['id'], "\"><i class=\"ti ti-trash\"></i></button>") + '</div>';
        }
      }],
      order: [[2, 'desc']],
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    $.get("".concat(baseUrl, "adherant/").concat(adherant_id, "/edit"), function (data) {
      ...
      $.ajax({
        type: 'GET',
        url: "".concat(baseUrl, "adherant/pays"),
  ...
  var formadhe = document.getElementById('demoForm');

  // user form validation
  var fv = FormValidation.formValidation(formadhe, {
    fields: {
      'prenom_adh': {
        validators: {
          notEmpty: {
    ...
    $.ajax({
      ...
      url: "".concat(baseUrl, "adherant/update"),
      type: 'POST',
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          data: 'id=' + adherant_id,
          url: "".concat(baseUrl, "adherant-destroy"),
          type: 'POST',
